// ฟังก์ชันช่วยจัดรูปแบบวันที่สำหรับแสดงในหัวตาราง
function formatDateLabel(dateData) {
  if (!dateData) return null;
  return `${dateData.date} (${dateData.dayOfWeek})`;
...
function saveWorkTimeData(formData) {
  try {
    ...
    if (!worktimeSheet) {
      ...
      for (let i = 1; i <= 14; i++) {
        const label = formatDateLabel(formData.dates[i-1]);
        headers.push(label || `วันที่ ${i}`);
      }
